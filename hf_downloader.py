...
import sys
import time
from huggingface_hub import HfApi, hf_hub_url, hf_hub_download, get_token
...
def fetch_files(repo_id, repo_type='model', token=None):
...
def select_files(files_info):
    ...
    choices = []
    # Add ALL option
    choices.append(("ALL (Download all files)", "ALL_FILES"))
...
def main():
    ...
    # Ask if user wants to download the default repo first
    if not args.repo_id:
    ...
    # Fetch files
    files_info = fetch_files(args.repo_id, repo_type=repo_type, token=token)
    if not files_info:
